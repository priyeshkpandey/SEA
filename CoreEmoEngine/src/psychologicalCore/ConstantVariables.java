...
public class ConstantVariables {
	...
	public static int simulationId = 1; // used but not initialized
	...
	public static String queryEmoAttByAgIDIter = "SELECT * FROM emo_attitudes WHERE iteration_no = ?"+
	                                             " AND agent_id = ? AND user_id = '"+ConstantVariables.userID+"' AND simulation_id = "+ConstantVariables.simulationId;
	public static String updSimDataWrkItrBySimID = "UPDATE simulation_data_tab SET working_iteration = ?"
			                                 +" WHERE (simulation_id = ? AND user_id = '"+ConstantVariables.userID+"' AND simulation_id = "+ConstantVariables.simulationId+")";
	...
	public static String queryAgentTable = "SELECT * FROM agent_tab WHERE iteration_no = ?"+
			                                   " AND agent_id = ? AND user_id = '"+ConstantVariables.userID+"'"
			                                   +" AND simulation_id = "+ConstantVariables.simulationId;
	public static String queryEventTab = "SELECT * FROM event_tab WHERE iteration_no = ?"
			                                   +" AND event_id = ? AND agent_id = ? AND user_id = '"+ConstantVariables.userID+"'"
			                                   +" AND simulation_id = "+ConstantVariables.simulationId;
	public static String queryAgentNeighbours = "SELECT * FROM agent_network WHERE iteration_no = ?"
			                                       +" AND agent_id1 = ? AND event_involved = ? AND user_id = '"+ConstantVariables.userID+"'"
			                                       +" AND simulation_id = "+ConstantVariables.simulationId;
	public static String queryAgentNetwork = "SELECT * FROM agent_network WHERE iteration_no = ?"
            +" AND agent_id1 = ? AND agent_id2 = ? AND user_id = '"+ConstantVariables.userID+"'"
			+" AND simulation_id = "+ConstantVariables.simulationId;
	public static String queryPrevState = "SELECT * FROM previous_state WHERE iteration_no = ? AND event_id = ? AND agent_id = ? AND emotion = '?'"
												+" AND user_id = '"+ConstantVariables.userID+"'"
												+" AND simulation_id = "+ConstantVariables.simulationId;
	public static String queryObjectTab = "SELECT * FROM object_tab WHERE iteration_no = ? AND object_id = ? AND agent_id = ? AND user_id = '"+ConstantVariables.userID+"'"
												+" AND simulation_id = "+ConstantVariables.simulationId;
	public static String insObservedEmos = "INSERT INTO observed_emotions VALUES(?, ?, ?, ?, ?, ?, ?, ?, '"+ConstantVariables.userID+"', "+ConstantVariables.simulationId+")";
	public static String insPrevState = "INSERT INTO previous_state VALUES(?, ?, ?, ?, ?, ?, '"+ConstantVariables.userID+"', "+ConstantVariables.simulationId+")";
	public static String queryVarMap = "SELECT * FROM variable_mappings WHERE variable_id = ?";
	public static String queryInteractModels = "SELECT * FROM interaction_attitudes WHERE iteration_no = ? AND agent_id1 = ? AND user_id = '"+ConstantVariables.userID+"'"
													+" AND simulation_id = "+ConstantVariables.simulationId;
