...
public class InitializeModelsAndInteract {
	...
	InitializeModelsAndInteract(int simId, int iter, int agent, int thrdPerson) {
		simulationId = simId;
		agentId = agent;
		currIter = iter;
	...
	public void initModels() {
		...
		try {
			...
			while (rsModels.next()) {
				...
				valueList.put("is_function", rsModels.getBoolean("is_function"));
			...
			while (rsVarMap.next()) {
				...
				tmpTableMetaData.add(rsVarMap.getString("is_interaction_involved"));
	...
	private void insertTableValues(String keyVal) throws SQLException
	{
		...
		if (!keyRowInserted.contains(keyVal)) {
			...
			StringBuffer insertQuery = new StringBuffer("INSERT INTO "
					+ tableName + "(");
			StringBuffer existentialQuery = new StringBuffer("SELECT * FROM "+tableName+" WHERE ");
			...
			for (int i = 0; i < noOfCondCols; i++) {
				...
				if (condColsArray[i].equalsIgnoreCase("iteration_no")) {
					...
					existentialQuery.append(condColsArray[i] + " = "+currIter+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("agent_id")
						|| condColsArray[i]
								.equalsIgnoreCase("agent_id1")) {
					insertQuery.append("" + agentId + ", ");
					existentialQuery.append(condColsArray[i] + " = "+sourceAgent+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("agent_id2")) {
					insertQuery.append("" + targetAgent + ", ");
					existentialQuery.append(condColsArray[i] + " = "+targetAgent+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("event_id")
						|| condColsArray[i]
								.equalsIgnoreCase("event_involved")
						|| condColsArray[i]
										.equalsIgnoreCase("target_event")) {
					insertQuery.append("" + targetEvent + ", ");
					existentialQuery.append(condColsArray[i] + " = "+targetEvent+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("emotions")
						|| condColsArray[i]
							.equalsIgnoreCase("target_emotion")) {
					insertQuery.append("'" + targetEmotion + "', ");
					existentialQuery.append(condColsArray[i] + " = "+targetEmotion+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("object_id")
						  || condColsArray[i]
							.equalsIgnoreCase("target_object")) {
					insertQuery.append("" + targetObject + ", ");
					existentialQuery.append(condColsArray[i] + " = "+targetObject+" AND");
				} else if (condColsArray[i].equalsIgnoreCase("user_id")) {
					insertQuery.append("'" + ConstantVariables.userID + "', ");
					existentialQuery.append(condColsArray[i] + " = "+ConstantVariables.userID+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("variable")) {
					insertQuery.append("'" + targetVariable + "', ");
					existentialQuery.append(condColsArray[i] + " = "+targetVariable+" AND");
				} else if (condColsArray[i]
						.equalsIgnoreCase("third_person")) {
					insertQuery.append("'" + thirdPerson + "', ");
					existentialQuery.append(condColsArray[i] + " = "+thirdPerson+" AND");
				}
			...
			boolean isRowExists = existsStmt.execute(existentialQuery.toString());
			existsStmt.close();
			...
			if (noOfRowsInserted > 0) {
				System.out.println("Successfully inserted in "
						+ tableName + " keys " + keyVal
						+ ". No of Rows " + noOfRowsInserted);
			...
			{
				System.out.println("Row already exists, no insert fired. Select query: "+existentialQuery.toString());
			}
	...
	private void setTableValues(String modelKey) throws ClassNotFoundException,
		...
		for (int i = 0; i < noOfCondCols; i++) {
			...
			if (condColsArray[i].equalsIgnoreCase("iteration_no")) {
				...
				updQuery.append(condColsArray[i] + " = " + sourceAgent + " AND ");
				...
				updQuery.append(condColsArray[i] + " = " + "'" + ConstantVariables.userID + "'"
						+ " AND ");
	...
	private void setInteractions()
	{
		try {
			...
			PreparedStatement psQueryInteract = conn.prepareStatement(ConstantVariables.queryInteractModels);
			...
			while(rsQueryInteract.next())
			{
				...
				if(isInteractVarModelExists(var))
				{
					...
					interactVals.add(rsQueryInteract.getDouble("interaction_probability"));
					interactVals.add(rsQueryInteract.getDouble("influence"));
					interactVals.add(rsQueryInteract.getDouble("influence_filter"));
					interactVals.add(rsQueryInteract.getDouble("influence_likelihood"));
					interactVals.add(rsQueryInteract.getDouble("influence_threshold"));
					interactVals.add(precedence);
					interactVals.add(rsQueryInteract.getBoolean("is_influence_persists"));
			...
			Iterator<String> sortedInteractionsKeys = sortedInteractionPrecedences.keySet().iterator();
			...
			while(sortedInteractionsKeys.hasNext())
			{
				...
				Boolean isInfluencePersists = (Boolean)interactionAttitudes.get(interactKey).get(6);
				
				if(isInfluencePersists)
				{
					
					int interactRange = (int)(1/(Double)interactionAttitudes.get(interactKey).get(0));
					
					boolean isProbable = (probGenerator.nextInt(interactRange) == (int)(interactRange/2));
					
					if(isProbable)
					{
						int influenceRange = (int)(1/(Double)interactionAttitudes.get(interactKey).get(6));
						boolean isInfluence = (probGenerator.nextInt(influenceRange) == (int)(influenceRange/2));
						
						if(isInfluence)
						{
							double influence = (Double)interactionAttitudes.get(interactKey).get(1);
							double influenceFilter = (Double)interactionAttitudes.get(interactKey).get(2);
							double influenceThreshold = (Double)interactionAttitudes.get(interactKey).get(4);
							double influenceIntensity = influenceThreshold - (influence - influenceFilter);
							...
							if(influenceIntensity > 0)
							{
								...
								String modelKey =   interactKey.split(",")[3]+","
												  ...
												  + interactKey.split(",")[5]+","
												  + interactKey.split(",")[6]+","
												  + interactKey.split(",")[7]+",,"// target variable should be null for interactions
	...
	{
		List <Entry<String, Integer>> sortedList = new LinkedList<Entry<String, Integer>>(interactionPrecedences.entrySet());
